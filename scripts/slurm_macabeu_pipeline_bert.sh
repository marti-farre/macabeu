#!/bin/bash
# SLURM array job: Full MACABEU pipeline for BERT
# generate data → train → offline eval → online eval
# 4 jobs (one per task)
#
# Submit with: cd ~/macabeu && sbatch scripts/slurm_macabeu_pipeline_bert.sh
...
#SBATCH -c 4
#SBATCH --array=0-3
#SBATCH -o logs/macabeu_bert_%A_%a.out
#SBATCH -e logs/macabeu_bert_%A_%a.err
...
MODEL_PATH="../BODEGA/data/$TASK/BERT-512.pth"
